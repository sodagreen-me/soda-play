...
soda-play/
├── public/             # 静态资源
│   └── images/album-covers/ # 已优化的专辑封面静态资源
├── src/
...
- **专辑封面**: 每个选项展示对应专辑封面，映射来自 `src/data/albumCovers.ts`
...
- **专辑封面**: 普通模式在题目提示区展示本题专辑封面；困难模式在每个选项上展示对应专辑封面
...
- 原始专辑封面放在根目录 `resources/album-cover/`，该目录已被 `.gitignore` 忽略，不应直接依赖或提交原图。
- 可上线使用的封面放在 `public/images/album-covers/`，当前为 360px 压缩 JPEG。
- 新增或替换封面时，先处理图片尺寸和体积，再放入 `public/images/album-covers/`。
- 游戏代码不要硬编码封面路径，应统一通过 `src/data/albumCovers.ts` 按专辑名映射。
- 当前需要封面的游戏：苏打AI猜、苏打AB面；苏打时间线不展示专辑封面。
